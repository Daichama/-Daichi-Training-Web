...
export default function Home() {
  ...
  async function finishWorkout() {
    ...
    try {
      const response = await fetch("/api/workouts", {
        ...
        body: JSON.stringify({ part, exercises, logs }),
      });
      ...
      setWorkout({ ...data.workout, saved: true });
      window.localStorage.removeItem(DRAFT_KEY);
      setTimerRunning(false);
      setTimerSeconds(0);
  ...
  return (
    <main>
      <header>
        <div className="eyebrow">DAICHI TRAINING v3</div>
                ...
                <article className="card exercise" key={exercise.id}>
                  ...
                  {(exercise.startWeight || exercise.rest || exercise.memo) && (
                    <div className="hint">
                      ...
                      {Number(exercise.rest) > 0 && <span>休憩 {exercise.rest}秒</span>}
      ...
      {timerRunning && (
        <section className="restTimer">
          ...
          <button onClick={() => setTimerSeconds((current) => current + 30)}>+30秒</button>
          <button
            onClick={() => {
              setTimerRunning(false);
              setTimerSeconds(0);
