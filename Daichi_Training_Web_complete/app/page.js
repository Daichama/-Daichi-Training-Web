...
export default function Home() {
  ...
  const volumeByPart = PARTS.map((partName) => ({
    part: partName,
    volume: (analytics?.history || []).filter((item) => item.part === partName).reduce((sum, item) => sum + Number(item.volume || 0), 0),
  }));
    ...
    <main>
      <header className="brandHeader">
        <div className="brandLockup">
          <DLogo />
          <div>
            <div className="eyebrow">D-log</div>
            <div className="brandTagline">MEASURE. IMPROVE. REPEAT.</div>
          </div>
        </div>
        <h1>{workout ? `${workout.part} Day` : appView === "history" ? "トレーニング履歴" : appView === "progress" ? "進捗" : "D-log"}</h1>
              ...
              {exercises.map((exercise, exerciseIndex) => {
                ...
                return (
                  <div className="previewRow" key={exercise.id}>
                    ...
                    <div className="previewActions"><span>{exercise.sets} sets</span><button onClick={() => moveExercise(exercise.id, -1)} disabled={exerciseIndex === 0} aria-label="上へ">↑</button><button onClick={() => moveExercise(exercise.id, 1)} disabled={exerciseIndex === exercises.length - 1} aria-label="下へ">↓</button><button className="previewDelete" onClick={() => skipExercise(exercise.id)} aria-label={`${exercise.name}を削除`}>削除</button></div>
                  </div>
                );
              })}
