...
export default function Home() {
  ...
  const currentWeekPlan = threeWeek.dates.map((date) => {
    ...
    <main>
      ...
      {!workout && (
        <>
          ...
          {appView === "home" && <>
          <section className="dashboard dashboardEnhanced card">
            <div className="dashboardMetric"><span className="dashboardIcon">◇</span><section><small>今月のトレ</small><strong>{monthWorkouts}回</strong><em>目標 16回／月</em><i><b style={{width:`${Math.min(100, monthWorkouts / 16 * 100)}%`}} /></i></section></div>
            <div className="dashboardMetric"><span className="dashboardIcon">◷</span><section><small>今月の時間</small><strong>{monthMinutes}分</strong><em>目標 720分／月</em><i><b style={{width:`${Math.min(100, monthMinutes / 720 * 100)}%`}} /></i></section></div>
            <div className="dashboardMetric"><span className="dashboardIcon">↗</span><section><small>今月のボリューム</small><strong>{formatVolume(monthVolume)}kg</strong><em>積み上げを継続中</em><i><b style={{width:`${Math.min(100, monthVolume / 60000 * 100)}%`}} /></i></section></div>
          </section>
          <section className="weeklyPlan card">
            <div className="weeklyPlanHead">
              <div><small>前後1週間の予定</small><strong>{threeWeek.start.slice(5).replace("-", "/")}–{threeWeek.end.slice(5).replace("-", "/")}</strong></div>
            </div>
