    ...
    <main>
      ...
      {!workout && (
        <>
          ...
          <section className="partRestGrid">
            {partRest.map((item) => <div className={`metric-card partRestCard partAccent ${partTone(item.part)}`} key={item.part}><small>{item.part}・前回から</small><strong className="viz-stat-value">{item.elapsed}</strong>{item.latest && <span>{item.latest.date}</span>}</div>)}
          </section>
                  ...
                  return <article className="card historyCard" id={`history-${item.id}`} key={item.id}>
                    ...
                    {open && <div className="historyDetail">
                      {editingHistoryId !== item.id ? <>
                        ...
                        <div className="historyMetaBar"><div className="historyMetaStats"><span>開始 <b>{meta.start || "—"}</b></span><span>終了 <b>{meta.end || "—"}</b></span><span>評価 <b>{meta.assessment || "—"}</b></span><span>パンプ <b>{meta.pump || "—"}</b></span><span>Session Score <b className="historyStars">{meta.sessionScore ? `${"★".repeat(Number(meta.sessionScore))}${"☆".repeat(5 - Number(meta.sessionScore))}` : "—"}</b></span><span>ストレッチ <b>{meta.stretch || "—"}</b></span><span>腹筋 <b>{meta.abs || "—"}</b></span></div><button className="secondary compact" onClick={() => beginHistoryEdit(item)}>✏️ 編集</button></div>
          ...
          {appView === "progress" && (
            <section className="analyticsPage">
              ...
              <nav className="progressTabs" aria-label="進捗カテゴリー">
                {[{id:"exercise",label:"種目"},{id:"volume",label:"ボリューム"},{id:"body",label:"Body"},{id:"photos",label:"写真"}].map((tab) => <button key={tab.id} className={progressTab === tab.id ? "active" : ""} onClick={() => setProgressTab(tab.id)}>{tab.label}</button>)}
              </nav>
          ...
          {appView === "menu" && <section className="menuPage"><button>テンプレート編集 <span>›</span></button><button onClick={() => { openView("history"); setEditingPlan(true); }}>週間予定の編集 <span>›</span></button><button>Bodyサイズの記録 <span>›</span></button><button>写真の管理 <span>›</span></button><button>設定 <span>›</span></button><small>D-log Ver.10.3.1</small></section>}
      ...
      {!workout && <footer className="appVersion">D-log Ver.10.3.1</footer>}
