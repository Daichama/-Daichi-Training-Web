...
const HISTORY_KEY = "daichi-training-v4-history";
...
export default function Home() {
  ...
  async function finishWorkout() {
    ...
    try {
      ...
      const historyEntry = { ...finishedSummary, date: workout?.date || todayInJapan() };
  ...
  const monthKey = todayInJapan().slice(0, 7);
  const monthHistory = history.filter((item) => String(item.date || "").startsWith(monthKey));
  const monthMinutes = monthHistory.reduce(
    (total, item) => total + Math.round((Number(item.durationSeconds) || 0) / 60),
    0
  );
  const monthVolume = monthHistory.reduce((total, item) => total + (Number(item.volume) || 0), 0);
  ...
  return (
    <main>
      <header>
        <div className="eyebrow">DAICHI TRAINING v4.0</div>
      ...
      {!workout && (
        <>
          <section className="dashboard card">
            <div><small>今月のトレ</small><strong>{monthHistory.length}回</strong></div>
