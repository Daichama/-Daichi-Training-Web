    ...
    <main>
      ...
      {!workout && (
        <>
          ...
          {appView === "menu" && <section className="menuPage"><button>テンプレート編集 <span>›</span></button><button onClick={() => { openView("history"); setEditingPlan(true); }}>週間予定の編集 <span>›</span></button><button>Bodyサイズの記録 <span>›</span></button><button>写真の管理 <span>›</span></button><button>設定 <span>›</span></button><small>D-log Ver.10.5.1</small></section>}
      ...
      {preWorkoutOpen && (
        <div className="modalBackdrop"><section className="summaryModal card preWorkoutModal">
          <small>PRE-WORKOUT</small><h2>ストレッチ時間</h2><p>実施した時間を選ぶだけでOK。</p>
          <div className="stretchChoices">{[5,10,15,20,30].map((minutes) => <button type="button" key={minutes} onClick={() => { setStretchSeconds(minutes * 60); launchWorkout("completed"); }}>{minutes}分</button>)}</div>
          <button className="secondary" onClick={skipStretchAndStart}>なしでトレ開始</button>
      ...
      {!workout && <footer className="appVersion">D-log Ver.10.5.1</footer>}
