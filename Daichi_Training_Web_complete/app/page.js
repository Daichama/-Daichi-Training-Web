...
export default function Home() {
  ...
  const volumeGoals = { 胸: 80000, 背中: 80000, 肩: 45000, 腕: 40000, 脚: 90000 };
  const volumeByPart = PARTS.map((partName) => {
    ...
    const goal = volumeGoals[partName] || 50000;
    return { part: partName, volume, sets, workouts: sessions.length, goal, rate: Math.min(100, Math.round(volume / goal * 100)) };
  });
  ...
  return (
    <main>
      <header className={`brandHeader ${!workout && appView === "home" ? "homeBrandHeader" : ""}`}>
        ...
        {workout && <><h1>{`${workout.part} Day`}</h1><div className="date">{workout.date}</div></>}
      ...
      {!workout && (
        <>
          ...
          <section className={`homeHero ${partTone(part || currentWeekPlan.find((day) => day.today)?.menu || "")}`}>
            ...
            {part && exercises.length > 0 && <button className="homeStartButton" onClick={startWorkout}>トレーニングを開始 <b>→</b></button>}
          </section>
          <section className="weeklyPlan card">
            ...
            <div className="weeklyPlanDays">
              {currentWeekPlan.map((day, index) => {
                ...
                return <button key={day.date} className={`weeklyPlanDay ${partTone(shownPart)}${day.today ? " today" : ""}${day.actual ? " done" : ""}`} onClick={() => { openView("history"); if (day.actual) setSelectedHistoryId(day.actual.id); }}>
                ...
                {historyMonths.map(([month, items], monthIndex) => {
                  ...
                  return <article className="card historyCard" id={`history-${item.id}`} key={item.id}>
                    <button className="historySummary" onClick={() => setSelectedHistoryId(open ? "" : item.id)}>
                      <div><small>{item.date}</small><strong>{item.part} Day</strong></div>
          ...
          {appView === "menu" && <section className="menuPage"><button>テンプレート編集 <span>›</span></button><button onClick={() => { openView("history"); setEditingPlan(true); }}>週間予定の編集 <span>›</span></button><button>部位別目標設定 <span>›</span></button><button>Bodyサイズの記録 <span>›</span></button><button>写真の管理 <span>›</span></button><button>設定 <span>›</span></button><small>D-log Ver.10.0.0</small></section>}
          <nav className="bottomNav" aria-label="メインナビゲーション">
            ...
            <button onClick={() => { openView("home"); document.querySelector('.parts')?.scrollIntoView({behavior:'smooth'}); }}><b>♙</b><span>ワークアウト</span></button>
        ...
        <div className="modalBackdrop">
          <section className="summaryModal card">
            ...
            <div className="summaryGrid">
              ...
              <div><span>パンプ</span><strong>{summary.pump || "—"}</strong></div>
