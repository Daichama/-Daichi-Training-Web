    ...
    <main>
      <header className="brandHeader">
        <div className="brandLockup">
          <DLogo />
          <div>
            <div className="eyebrow">D-log · v9.0.0</div>
                ...
                {(analytics?.history || []).map((item) => {
                  ...
                  return <article className="card historyCard" key={item.id}>
                    ...
                    {open && <div className="historyDetail">
                      {item.exercises.map((exercise) => <div className="historyExercise" key={exercise.id}><strong>{exercise.name}</strong><div>{exercise.sets.map((set) => <span key={set.id}>{set.weight}kg × {set.reps}{set.rir ? ` · RIR${set.rir}` : ""}</span>)}</div></div>)}
