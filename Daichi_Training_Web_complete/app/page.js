...
export default function Home() {
    ...
    try {
      const response = await fetch("/api/workouts", {
        ...
        body: JSON.stringify({
          ...
          durationMinutes,
          sessionMemo: [sessionMemo, `Stretch: ${stretchStatus === "completed" ? `${stretchSeconds}秒` : "Skip"}`, `Abs: ${absStatus === "completed" ? absSets.map((set) => `${set.reps}回`).join(" / ") : "Skip"}`].filter(Boolean).join("\n"),
          exerciseNotes,
          ratings: {
  ...
  const volumeByPart = PARTS.map((partName) => {
    const sessions = (analytics?.history || []).filter((item) => item.part === partName);
    ...
    const target = hardSetTargets[partName] || [10, 16];
    return { part: partName, volume, sets, workouts: sessions.length, target, rate: Math.min(100, Math.round(sets / target[1] * 100)) };
  });
  const homeScheduleDays = getHomeScheduleWindow(weeklyPlan, todayInJapan(), 6);
  const adherenceDays = homeScheduleDays.filter((day) => day.date <= todayInJapan() && day.menu);
  const adherence = adherenceDays.reduce((acc, day) => {
    const actual = historyByDate.get(day.date);
    if (day.menu === "休み" && !actual) acc.onPlan += 1;
    else if (actual && actual.part === day.menu) acc.onPlan += 1;
    else if (actual || day.date < todayInJapan()) acc.changed += 1;
    return acc;
  }, { onPlan: 0, changed: 0 });
    ...
    <main>
      ...
      {!workout && (
        <>
          ...
          {appView === "home" && <>
          <section className={`todayWorkoutCard card ${partTone(part || currentWeekPlan.find((day) => day.today)?.menu || "")}`}>
            <div><small>今日のトレーニング</small><strong>{part || currentWeekPlan.find((day) => day.today)?.menu || "メニューを選択"}</strong><span>{part ? `${exercises.length}種目` : "下から部位を選択"}</span></div>
            {part && exercises.length > 0 && <button className="homeStartButton" onClick={beginPreWorkout}>開始する <b>→</b></button>}
          </section>
          <section className="monthlyStrip card">
            <div className="weeklyPlanHead"><div><small>今月の予定</small><strong>{calendarMonth.replace("-", "年")}月</strong></div><button className="planEditButton" onClick={() => { openView("history"); setEditingPlan(true); }}>編集</button></div>
            <div className="monthlySwipe" aria-label="前後の予定を横にスワイプ">{homeScheduleDays.map((day) => {
              const actual = historyByDate.get(day.date);
              const isRestOnPlan = day.menu === "休み" && day.date < todayInJapan() && !actual;
              const isDone = actual && actual.part === day.menu;
              const isChanged = actual && actual.part !== day.menu;
              const isMissed = day.date < todayInJapan() && day.menu && day.menu !== "休み" && !actual;
              const shownPart = actual?.part || day.menu;
              return <button key={day.date} className={`monthStripDay ${partTone(shownPart)}${day.date === todayInJapan() ? " today" : ""}${isDone ? " done" : ""}${isRestOnPlan ? " restOnPlan" : ""}${isChanged ? " changed" : ""}${isMissed ? " missed" : ""}`} onClick={() => { openView("history"); if (actual) setSelectedHistoryId(actual.id); }}>
                <span>{["日","月","火","水","木","金","土"][new Date(`${day.date}T00:00:00+09:00`).getDay()]}</span><em>{Number(day.date.slice(8))}</em><strong>{shownPart || ""}</strong>{isDone && <i>済</i>}{isRestOnPlan && <i>予定通り</i>}{isChanged && <i>変更</i>}{isMissed && <i>未実施</i>}
              </button>;
            })}</div>
            <div className="planAdherence"><div className="adherenceRing" style={{"--rate": `${adherenceDays.length ? Math.round(adherence.onPlan / adherenceDays.length * 100) : 0}%`}}><strong>{adherenceDays.length ? Math.round(adherence.onPlan / adherenceDays.length * 100) : 0}%</strong></div><div><strong>計画どおり進行中</strong><span>予定どおり {adherence.onPlan}日 ・ 変更 {adherence.changed}日</span></div></div>
          </section>
          ...
          {exercises.length > 0 && (
            <section className="preview card">
              ...
              <button className="primary" onClick={beginPreWorkout} disabled={busy}>
                {`${part}を開始`}
              </button>
            </section>
          )}
                ...
                {historyMonths.map(([month, items], monthIndex) => {
                  ...
                  return <article className="card historyCard" id={`history-${item.id}`} key={item.id}>
                    <button className="historySummary" onClick={() => setSelectedHistoryId(open ? "" : item.id)}>
                      <div><small>{item.date}</small><strong>{item.part}</strong></div>
                    ...
                    {open && <div className="historyDetail">
                      {editingHistoryId !== item.id ? <>
                        <div className="historyEditBar"><button className="secondary compact" onClick={() => beginHistoryEdit(item)}>✏️ 編集</button></div>
                        <div className="historyTableWrap"><table className="historyTable"><thead><tr><th>種目</th>{Array.from({length: Math.max(0, ...item.exercises.map((exercise) => exercise.sets.length))}, (_, index) => <th key={index}>{index + 1}セット目</th>)}</tr></thead><tbody>{item.exercises.map((exercise, exerciseIndex) => <tr key={exercise.id}><th>{exerciseIndex + 1}. {exercise.name}</th>{Array.from({length: Math.max(0, ...item.exercises.map((row) => row.sets.length))}, (_, index) => { const set = exercise.sets[index]; return <td key={index}>{set ? <><strong>{set.weight}kg×{set.reps}回</strong>{set.rir !== "" && set.rir != null ? <small>RIR {set.rir}</small> : null}</> : <span>—</span>}</td>; })}</tr>)}</tbody></table></div>
                        {item.memo && <p className="historyMemo">{item.memo}</p>}
          ...
          {appView === "progress" && (
            <section className="analyticsPage">
              ...
              {!analyticsBusy && analytics && <>
                {progressTab === "exercise" && <section className="card smartCoach"><small>SMART COACH</small><h3>次回への提案</h3>{smartTips.length ? smartTips.map((tip) => <p key={tip}>{tip}</p>) : <p>記録が2回以上たまると提案を表示するで。</p>}</section>}
                {progressTab === "volume" && <section className="card volumeUse"><small>VOLUME BY PART</small><h3>部位別総ボリューム</h3>{volumeByPart.map((item) => <div className="volumeDetailRow" key={item.part}><div className="volumeDetailHead"><strong>{item.part}</strong><span>{item.workouts}回 / {item.sets}セット</span></div><div className="volumeTrack"><i style={{width: `${item.rate}%`}} /></div><div className="volumeDetailFoot"><span>{formatVolume(item.volume)}kg</span><em>目安 {item.target[0]}〜{item.target[1]}ハードセット/週</em><b>{item.sets}セット</b></div></div>)}</section>}
          ...
          {appView === "menu" && <section className="menuPage"><button>テンプレート編集 <span>›</span></button><button onClick={() => { openView("history"); setEditingPlan(true); }}>週間予定の編集 <span>›</span></button><button>Bodyサイズの記録 <span>›</span></button><button>写真の管理 <span>›</span></button><button>設定 <span>›</span></button><small>D-log Ver.10.0.0</small></section>}
        ...
        <div className="modalBackdrop">
          <section className="summaryModal card">
            ...
            <div className="summaryGrid">
              ...
              <div><span>パンプ</span><strong>{summary.pump || "—"}</strong></div><div><span>ストレッチ</span><strong>{summary.stretchStatus === "completed" ? formatTimer(summary.stretchSeconds) : "Skip"}</strong></div><div><span>腹筋</span><strong>{summary.absStatus === "completed" ? summary.absSets.map((set) => `${set.reps}回`).join(" / ") : "Skip"}</strong></div>
      ...
      {!workout && <footer className="appVersion">D-log Ver.10.0.0</footer>}
