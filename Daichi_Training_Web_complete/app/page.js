...
export default function Home() {
  ...
  return (
    <main>
      ...
      {!workout && (
        <>
          ...
          {appView === "coach" && <section className="coachPage analyticsPage">
            ...
            <section className={`card smartCoach partAccent ${partTone(nextCoachPart)}`}><small>NEXT SESSION</small><h3>{nextCoachPlan ? `${nextCoachPlan.date.slice(5).replace("-", "/")}・${nextCoachPart}` : nextCoachPart}</h3>{partCoachTips.length ? partCoachTips.map((tip) => <p key={tip}>{tip}</p>) : <p>{nextCoachPart}の記録がたまると、次回に絞った提案を表示するで。</p>}</section>
            <section className="card coachPriority"><small>PRIORITY</small><h3>次回の優先順位</h3>{partCoachTips.slice(0, 3).map((tip, index) => <div key={tip}><b>{index + 1}</b><span>{tip}</span></div>)}</section>
          </section>}
          {appView === "menu" && <section className="menuPage"><button>テンプレート編集 <span>›</span></button><button onClick={() => { openView("history"); setEditingPlan(true); }}>週間予定の編集 <span>›</span></button><button>Bodyサイズの記録 <span>›</span></button><button>写真の管理 <span>›</span></button><button>設定 <span>›</span></button><small>D-log Ver.10.7.0</small></section>}
      ...
      {!workout && <footer className="appVersion">D-log Ver.10.7.0</footer>}
