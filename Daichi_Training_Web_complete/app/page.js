    ...
    <main>
      ...
      {!workout && (
        <>
          ...
          {appView === "progress" && (
            <section className="analyticsPage">
              ...
              <nav className="progressTabs" aria-label="進捗カテゴリー">
                {[{id:"recovery",label:"部位間隔"},{id:"exercise",label:"種目"},{id:"volume",label:"ボリューム"},{id:"body",label:"Body"},{id:"diary",label:"Diary"}].map((tab) => <button key={tab.id} className={progressTab === tab.id ? "active" : ""} onClick={() => setProgressTab(tab.id)}>{tab.label}</button>)}
              </nav>
              ...
              {!analyticsBusy && analytics && <>
                {progressTab === "recovery" && <section className="recoverySection"><div className="recoveryIntro"><small>RECOVERY STATUS</small><h3>前回トレーニングからの経過</h3><p>部位ごとに、最後に実施してからの時間をまとめて確認できるで。</p></div><div className="partRestGrid progressPartRest">{partRest.map((item) => { const hours = item.latest ? Math.floor((now - workoutFinishedAt(item.latest)) / 3600000) : null; const status = hours == null ? "未実施" : hours < 48 ? "回復中" : hours < 96 ? "そろそろ" : hours < 168 ? "狙い目" : "かなり空き"; return <article className={`metric-card partRestCard partAccent ${partTone(item.part)}`} key={item.part}><div className="recoveryCardHead"><strong>{item.part}</strong><span>{status}</span></div><b className="viz-stat-value">{item.elapsed}</b><small>{item.latest ? `前回：${item.latest.date} ${extractWorkoutMeta(item.latest.memo).end || extractWorkoutMeta(item.latest.memo).start || ""}` : "まだ記録がないで"}</small></article>})}</div></section>}
                ...
                {progressTab === "diary" && <section className="card diaryTracking"><small>DAILY NOTE</small><h3>シンプル・ダイアリー</h3><div className="diaryGrid"><input type="date" value={diaryDraft.date} onChange={(e) => setDiaryDraft((d) => ({...d,date:e.target.value}))}/><input placeholder="体重 kg（任意）" inputMode="decimal" value={diaryDraft.weight} onChange={(e) => setDiaryDraft((d) => ({...d,weight:e.target.value}))}/><select value={diaryDraft.mood} onChange={(e) => setDiaryDraft((d) => ({...d,mood:e.target.value}))}><option>😊</option><option>🙂</option><option>😐</option><option>😫</option></select><input placeholder="写真URL（任意）" value={diaryDraft.photo} onChange={(e) => setDiaryDraft((d) => ({...d,photo:e.target.value}))}/><textarea placeholder="今日のひとこと" value={diaryDraft.note} onChange={(e) => setDiaryDraft((d) => ({...d,note:e.target.value}))}/></div><button className="primary compact" onClick={saveDiaryRecord}>保存</button><div className="diaryRecords">{diaryRecords.slice(0,12).map((record) => <article key={record.id}><div><strong>{record.date}</strong><span>{record.mood} {record.weight ? `${record.weight}kg` : ""}</span></div><p>{record.note || "—"}</p>{record.photo && <a href={record.photo} target="_blank" rel="noreferrer">写真を見る</a>}</article>)}</div></section>}
              </>}
          ...
          {appView === "menu" && <section className="menuPage"><button>テンプレート編集 <span>›</span></button><button onClick={() => { openView("history"); setEditingPlan(true); }}>週間予定の編集 <span>›</span></button><button>Bodyサイズの記録 <span>›</span></button><button>写真の管理 <span>›</span></button><button>設定 <span>›</span></button><small>D-log Ver.10.4.0</small></section>}
      ...
      {!workout && <footer className="appVersion">D-log Ver.10.4.0</footer>}
