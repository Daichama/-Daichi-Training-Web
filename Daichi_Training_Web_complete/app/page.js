    ...
    <main>
      ...
      {!workout && (
        <>
          ...
          {appView === "home" && <>
          <section className={`homeHero ${partTone(part || currentWeekPlan.find((day) => day.today)?.menu || "")}`}>
            <div className="homeHeroVisual" aria-hidden="true"><span></span><i></i></div>
            <div className="homeHeroContent"><small>TODAY</small><strong>{part || currentWeekPlan.find((day) => day.today)?.menu || "WORKOUT"}</strong><span>{part ? `${exercises.length}種目を準備中` : "今日のメニューを選択"}</span></div>
            {part && exercises.length > 0 && <button className="homeStartButton" onClick={beginPreWorkout}>トレーニングを開始 <b>→</b></button>}
          </section>
          <section className="weeklyPlan card">
            <div className="weeklyPlanHead">
              <div><small>THIS WEEK</small><strong>{currentWeek.start.slice(5).replace("-", "/")}–{currentWeek.end.slice(5).replace("-", "/")}</strong></div>
            ...
            <div className="weeklyPlanDays">
              {currentWeekPlan.map((day, index) => {
                ...
                const isChanged = day.actual && day.actual.part !== day.menu;
                return <button key={day.date} className={`weeklyPlanDay ${partTone(shownPart)}${day.today ? " today" : ""}${day.actual ? " done" : ""}${isChanged ? " changed" : ""}${isRestOnPlan ? " restOnPlan" : ""}`} onClick={() => { openView("history"); if (day.actual) setSelectedHistoryId(day.actual.id); }}>
