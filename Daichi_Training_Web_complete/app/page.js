                  ...
                  return <article className="card historyCard" id={`history-${item.id}`} key={item.id}>
                    ...
                    {open && <div className="historyDetail">
                      {editingHistoryId !== item.id ? <>
                        ...
                        <p className="historyEditHint">左の☰を長押しして上下へドラッグ。重量・回数・RIRも直接修正できるで。</p>
                        {historyDraft.exercises.map((exercise, exerciseIndex) => <section data-history-exercise-index={exerciseIndex} className={`historyEditExercise ${draggingHistoryIndex === exerciseIndex ? "isDragging" : ""}`} key={exercise.exerciseId || `${exercise.name}-${exerciseIndex}`}>
                          <div className="historyEditExerciseHead"><button type="button" className="historyDragHandle" aria-label={`${exercise.name}を並び替え`} onPointerDown={(event) => startHistoryExerciseDrag(event, exerciseIndex)} onPointerMove={moveHistoryExerciseDrag} onPointerUp={endHistoryExerciseDrag} onPointerCancel={endHistoryExerciseDrag}>☰</button><strong>{exerciseIndex + 1}. {exercise.name}</strong><div><button className="dangerText" onClick={() => removeHistoryExercise(exerciseIndex)}>種目削除</button></div></div>
