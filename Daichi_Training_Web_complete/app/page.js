...
function extractWorkoutMeta(memo) {
  ...
  const stretch = text.match(/Stretch[：:]\s*([^\n]+)/i)?.[1]?.trim() || "";
  const abs = text.match(/Abs[：:]\s*([^\n]+)/i)?.[1]?.trim() || "";
  ...
  const cleanMemo = text
    ...
    .replace(/\s*\/?\s*(?:Stretch|Abs|Session Score)[：:]\s*[^\n]+/gi, "")
    .replace(/^\s*[\/\n]+|[\/\n]+\s*$/g, "")
    .trim();
    ...
    <main>
      ...
      {!workout && (
        <>
          ...
          {appView === "menu" && <section className="menuPage"><button>テンプレート編集 <span>›</span></button><button onClick={() => { openView("history"); setEditingPlan(true); }}>週間予定の編集 <span>›</span></button><button>Bodyサイズの記録 <span>›</span></button><button>写真の管理 <span>›</span></button><button>設定 <span>›</span></button><small>D-log Ver.10.3.0</small></section>}
      ...
      {!workout && <footer className="appVersion">D-log Ver.10.3.0</footer>}
