    ...
    <main>
      ...
      {!workout && (
        <>
                  ...
                  return <article className="card historyCard" id={`history-${item.id}`} key={item.id}>
                    ...
                    {open && <div className="historyDetail">
                      {editingHistoryId !== item.id ? <>
                        ...
                        <div className="historyLocation"><span>実施場所</span><strong>{extractWorkoutLocation(item.memo) || "PHGノース大阪"}</strong></div>
          ...
          {appView === "menu" && <section className="menuPage"><button>テンプレート編集 <span>›</span></button><button onClick={() => { openView("history"); setEditingPlan(true); }}>週間予定の編集 <span>›</span></button><button>Bodyサイズの記録 <span>›</span></button><button>写真の管理 <span>›</span></button><button>設定 <span>›</span></button><small>D-log Ver.10.1.2</small></section>}
      ...
      {!workout && <footer className="appVersion">D-log Ver.10.1.2</footer>}
