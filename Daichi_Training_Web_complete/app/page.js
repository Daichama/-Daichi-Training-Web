...
const DIARY_KEY = "d-log-v10-diary";
...
function getThreeWeekRange() {
  ...
  const monday = new Date(`${current.start}T00:00:00+09:00`);
  ...
  return { start: dates[0], end: dates[20], dates };
}

function workoutFinishedAt(item) {
  if (!item?.date) return 0;
  const meta = extractWorkoutMeta(item.memo);
  const time = meta.end || meta.start || "00:00";
  const timestamp = Date.parse(`${item.date}T${time}:00+09:00`);
  return Number.isFinite(timestamp) ? timestamp : 0;
}

function formatPartRest(milliseconds) {
  if (!Number.isFinite(milliseconds) || milliseconds < 0) return "—";
  const totalHours = Math.floor(milliseconds / 3600000);
  return `${Math.floor(totalHours / 24)}日 ${totalHours % 24}時間`;
}
...
function extractWorkoutMeta(memo) {
  ...
  const sessionScore = text.match(/Session Score[：:]\s*([1-5])/i)?.[1] || "";
  const stretch = text.match(/(?:Stretch|ストレッチ)[：:]\s*([^\n]+)/i)?.[1]?.trim() || "";
  const abs = text.match(/(?:Abs|腹筋)[：:]\s*([^\n]+)/i)?.[1]?.trim() || "";
  ...
  const cleanMemo = text
    ...
    .replace(/\s*\/?\s*(?:Stretch|ストレッチ|Abs|腹筋|Session Score)[：:]\s*[^\n]+/gi, "")
    .replace(/^\s*[\/\n]+|[\/\n]+\s*$/g, "")
    .trim();
  return { start, end, assessment, pump, sessionScore, stretch, abs, location, cleanMemo };
}


function buildWorkoutMemo(draft) {
  const meta = extractWorkoutMeta(draft.memo || "");
  const clean = meta.cleanMemo || "";
  const lines = [
    draft.location ? `場所：${String(draft.location).trim()}` : "",
    draft.start || draft.end ? `開始 ${draft.start || "--:--"} / 終了 ${draft.end || "--:--"}` : "",
    draft.assessment ? `評価：${draft.assessment}` : "",
    draft.pump ? `パンプ：${draft.pump}` : "",
    draft.sessionScore ? `Session Score: ${draft.sessionScore}` : "",
    draft.stretch ? `Stretch: ${draft.stretch}` : "",
    draft.abs ? `Abs: ${draft.abs}` : "",
    `最終更新：${new Intl.DateTimeFormat("ja-JP", { timeZone: "Asia/Tokyo", year: "numeric", month: "2-digit", day: "2-digit", hour: "2-digit", minute: "2-digit", hour12: false }).format(new Date())}`,
    clean,
  ].filter(Boolean);
  return lines.join("\n");
}
...
function cleanExerciseLogMemo(memo) {
  return String(memo || "")
    .split(" / ")
    .filter((piece) => piece && !/^前回：/.test(piece) && !/^レストポーズ /.test(piece) && !/^休憩 /.test(piece) && !/^休憩理由：/.test(piece) && !/^Ready：/.test(piece) && !/^\[DLOG_EXERCISE_ORDER=/.test(piece))
    .join(" / ")
    .trim();
}
...
export default function Home() {
  ...
  const [coachPart, setCoachPart] = useState("");
  ...
  const [progressTab, setProgressTab] = useState("recovery");
  ...
  const [diaryDraft, setDiaryDraft] = useState({ date: todayInJapan(), note: "", weight: "", mood: "🙂", photo: "" });
  ...
  const weeklyPlanScrollRef = useRef(null);
  const workoutPreviewRef = useRef(null);
  ...
  async function beginHistoryEdit(item) {
    ...
    setHistoryDraft({
      ...item,
      location: meta.location || "PHGノース大阪",
      start: meta.start,
      end: meta.end,
      assessment: meta.assessment,
      pump: meta.pump,
      sessionScore: meta.sessionScore,
    ...
    try {
      const response = await fetch("/api/history-edit", {
        ...
        body: JSON.stringify({
          ...
          workoutMemo: buildWorkoutMemo(historyDraft),
  ...
  async function refreshWeeklyPlan(month = calendarMonth) {
    try {
      ...
      // Notion同期が使える時はサーバー側を唯一の正として端末差をなくす。
      // fallback時だけ、この端末の未同期ローカル予定を重ねる。
      if (data.source === "notion") {
        setWeeklyPlan(serverDays);
      } else {
        let localDays = [];
  ...
  async function refreshHomePlan() {
    try {
      const range = getThreeWeekRange();
      const months = [...new Set(range.dates.map((date) => date.slice(0, 7)))];
      const responses = await Promise.all(months.map(async (month) => {
        const response = await fetch(`/api/schedule?month=${encodeURIComponent(month)}`, { cache: "no-store" });
        const data = await response.json();
        if (!response.ok) throw new Error(data.error || "予定を読み込めなかった。");
        return { days: Array.isArray(data.days) ? data.days : [], source: data.source || "fallback" };
      }));
      let localDays = [];
      try { localDays = JSON.parse(window.localStorage.getItem(PLAN_KEY) || "[]"); } catch {}
      const localMap = new Map((Array.isArray(localDays) ? localDays : []).map((day) => [day.date, day.menu]));
      const wanted = new Set(range.dates);
      const merged = responses.flatMap(({ days, source }) => days.map((day) => ({ ...day, source }))).filter((day) => wanted.has(day.date));
      setHomePlan(merged.map((day) => ({
        ...day,
        menu: day.source === "notion" ? (day.menu || "") : (localMap.has(day.date) ? localMap.get(day.date) : (day.menu || "")),
      })));
    } catch (error) {
      setMessage(error instanceof Error ? error.message : "ホーム予定の読み込みに失敗したで。");
    }
  }
  ...
  useEffect(() => {
    ...
    try {
      ...
      const savedDiary = JSON.parse(window.localStorage.getItem(DIARY_KEY) || "[]");
      if (Array.isArray(savedDiary)) setDiaryRecords(savedDiary);
    } catch {}
  }, []);
  ...
  useEffect(() => {
    ...
    return () => { window.removeEventListener("focus", syncPlans); document.removeEventListener("visibilitychange", syncPlans); };
  }, [calendarMonth]);

  useEffect(() => {
    if ((!workout || workout.saved) && stretchStatus !== "running") return;
    const intervalId = window.setInterval(() => setNow(Date.now()), 1000);
    return () => window.clearInterval(intervalId);
  }, [workout, stretchStatus]);
  ...
  async function selectPart(value, selectedVariant = variant) {
    ...
    try {
      ...
      setBusy(false);
      // iOS SafariではDOM差し替え後でないと、開始カードへのスクロールが
      // 無視されることがあるため、描画完了を待ってから移動する。
      window.setTimeout(() => {
        workoutPreviewRef.current?.scrollIntoView({ behavior: "smooth", block: "start" });
      }, 120);
    }
  }
  ...
  function changePreviewSetCount(exerciseId, delta) {
    setExercises((current) => current.map((exercise) => {
      if (exercise.id !== exerciseId) return exercise;
      const nextSets = Math.max(1, Math.min(10, (Number(exercise.sets) || 1) + delta));
  ...
  function startStretch() {
    ...
    setStretchStartedAt(Date.now() - stretchSeconds * 1000);
    setNow(Date.now());
  }

  function pauseStretch() {
    if (!stretchStartedAt) return;
    setStretchSeconds(Math.max(0, Math.floor((Date.now() - stretchStartedAt) / 1000)));
    setStretchStartedAt(null);
    setStretchStatus("paused");
  }
    ...
    try {
      const response = await fetch("/api/workouts", {
        ...
        body: JSON.stringify({
          ...
          durationMinutes,
          sessionMemo: [sessionMemo, saveConfirm.location ? `場所: ${saveConfirm.location}` : "", `Session Score: ${saveConfirm.sessionScore || 0}`, `Stretch: ${stretchStatus === "completed" ? `${stretchSeconds}秒` : "Skip"}`, `Abs: ${absStatus === "completed" ? absSets.map((set) => `${set.reps}回`).join(" / ") : "Skip"}`].filter(Boolean).join("\n"),
          exerciseNotes,
          ratings: {
      ...
      const finishedSummary = {
        ...
        assessment: saveConfirm.assessment || "",
        pump: saveConfirm.pump || "",
        sessionScore: Number(saveConfirm.sessionScore) || 0,
        location: saveConfirm.location || "",
        stretchSeconds,
        stretchStatus,
        absStatus,
  ...
  function openView(view) {
    setAppView(view);
    if (view === "history" || view === "progress" || view === "coach") loadAnalytics();
  }
  ...
  useEffect(() => {
    ...
    const frame = window.requestAnimationFrame(() => {
      const scroller = weeklyPlanScrollRef.current;
      const todayButton = scroller?.querySelector?.(".weeklyPlanDay.today");
      if (scroller && todayButton) {
        scroller.scrollLeft = Math.max(0, todayButton.offsetLeft - scroller.clientWidth / 2 + todayButton.clientWidth / 2);
      }
    });
    return () => window.cancelAnimationFrame(frame);
  }, [appView, homePlan.length, analytics?.history?.length]);
  ...
  const threeWeek = getThreeWeekRange();
  const threeWeekPlan = threeWeek.dates.map((date) => {
    const day = homePlan.find((item) => item.date === date);
    const actual = (analytics?.history || []).find((item) => item.date === date);
    return { date, menu: day?.menu || "", today: date === todayInJapan(), actual };
  });
  const currentWeekPlan = threeWeekPlan.filter((day) => currentWeek.dates.includes(day.date));
  const currentWeekTargets = currentWeekPlan.filter((day) => day.menu && day.menu !== "休み").length;
  const currentWeekCompleted = currentWeekPlan.filter((day) => day.actual).length;
  const partRest = PARTS.map((partName) => {
    const latest = (analytics?.history || [])
      .filter((item) => item.part === partName)
      .sort((a, b) => workoutFinishedAt(b) - workoutFinishedAt(a))[0];
    const finishedAt = workoutFinishedAt(latest);
    return { part: partName, latest, elapsed: finishedAt ? formatPartRest(now - finishedAt) : "未実施" };
  });
  ...
  const today = todayInJapan();
  const todayCompleted = (analytics?.history || []).some((item) => item.date === today);
  const nextCoachPlan = [...homePlan]
    .filter((item) => (item.date > today || (item.date === today && !todayCompleted)) && PARTS.includes(item.menu))
    .sort((a, b) => a.date.localeCompare(b.date))[0];
  const autoCoachPart = nextCoachPlan?.menu || part || PARTS[0];
  const nextCoachPart = coachPart || autoCoachPart;
  const coachExercises = (analytics?.exercises || []).filter((item) => item.part === nextCoachPart);
  const coachLatestWorkout = (analytics?.history || []).filter((item) => item.part === nextCoachPart).sort((a,b) => b.date.localeCompare(a.date))[0];
  const coachDaysSince = coachLatestWorkout ? Math.max(0, Math.floor((Date.now() - workoutFinishedAt(coachLatestWorkout)) / 86400000)) : null;
  const coachMonthly = volumeByPart.find((item) => item.part === nextCoachPart);
  const partCoachTips = coachExercises.slice(0, 6).map((exercise) => {
    ...
    if (delta > 0.2) return `${exercise.name}：e1RM +${delta.toFixed(1)}kg。次回は同重量で+1回、余裕があれば小幅増量。`;
    if (delta < -0.2) return `${exercise.name}：e1RM ${delta.toFixed(1)}kg。重量維持でフォームと回数の回復を優先。`;
    return `${exercise.name}：直近2回は横ばい。まず1セットだけ回数更新を狙う。`;
  });
  ...
  function saveDiaryRecord() {
    const next = { ...diaryDraft, id: `${diaryDraft.date}-${Date.now()}` };
    const records = [next, ...diaryRecords.filter((item) => item.date !== diaryDraft.date)].sort((a, b) => b.date.localeCompare(a.date));
    setDiaryRecords(records);
    window.localStorage.setItem(DIARY_KEY, JSON.stringify(records));
    setMessage("ダイアリーを保存したで。");
  }
  ...
  return (
    <main>
      <header className={`brandHeader ${!workout && appView === "home" ? "homeBrandHeader" : ""}`}>
        ...
        {!workout && appView !== "home" && <h1>{appView === "history" ? "トレーニング履歴" : appView === "progress" ? "進捗" : appView === "coach" ? "Smart Coach" : "メニュー"}</h1>}
      </header>
      ...
      {!workout && (
        <>
          ...
          {appView === "home" && <>
          <section className="dashboard dashboardEnhanced homeCompactMetrics card">
            <div className="dashboardMetric"><span className="dashboardIcon">◇</span><section><small>今月</small><strong>{monthWorkouts}<em> / 20回</em></strong><i><b style={{width:`${Math.min(100, monthWorkouts / 20 * 100)}%`}} /></i></section></div>
            <div className="dashboardMetric"><span className="dashboardIcon">✓</span><section><small>今週達成</small><strong>{currentWeekCompleted}<em> / {currentWeekTargets || 5}回</em></strong><i><b style={{width:`${Math.min(100, currentWeekCompleted / Math.max(1, currentWeekTargets || 5) * 100)}%`}} /></i></section></div>
            <div className="dashboardMetric"><span className="dashboardIcon">↗</span><section><small>月間Volume</small><strong>{formatVolume(monthVolume)}<em>kg</em></strong><span className="metricSub">時間 {monthMinutes}分</span></section></div>
          </section>
          <section className="weeklyPlan card">
            <div className="weeklyPlanHead">
              <div><small>今週の予定</small><strong>{currentWeek.start.slice(5).replace("-", "/")}–{currentWeek.end.slice(5).replace("-", "/")}</strong></div>
            </div>
            <div className="weeklyPlanDays" ref={weeklyPlanScrollRef}>
              {threeWeekPlan.map((day, index) => {
                const shownPart = day.actual?.part || day.menu;
                const isStrategicRest = day.menu === "休み" && day.date <= todayInJapan() && !day.actual;
                const isMissed = day.date < todayInJapan() && day.menu && day.menu !== "休み" && !day.actual;
                return <button key={day.date} className={`weeklyPlanDay ${partTone(shownPart)}${day.today ? " today" : ""}${day.actual ? " done" : ""}${isStrategicRest ? " strategicRest" : ""}${isMissed ? " missed" : ""}`} onClick={() => { openView("history"); if (day.actual) setSelectedHistoryId(day.actual.id); }}>
                  <span>{["日","月","火","水","木","金","土"][new Date(`${day.date}T00:00:00+09:00`).getDay()]}</span>
                  <em>{Number(day.date.slice(8))}</em>
                  <strong>{shownPart || ""}</strong>
                  {day.actual && <i className="miniDoneStamp">済</i>}
                  {isStrategicRest && <i className="miniDoneStamp miniRestStamp" aria-label="戦略的休息">♥</i>}
          ...
          {busy && part && <section className="preview card workoutPreviewLoading" ref={workoutPreviewRef}><strong>{part}メニューを読み込み中…</strong></section>}
          {!busy && exercises.length > 0 && (
            <section className="preview card" ref={workoutPreviewRef}>
              {exercises.map((exercise, exerciseIndex) => {
                ...
                return (
                  <div className={`previewRow ${draggingPreviewIndex === exerciseIndex ? "isDragging" : ""}`} key={exercise.id} data-preview-index={exerciseIndex}>
                    ...
                    <div className="previewActions"><div className="previewSetStepper" aria-label={`${exercise.name}のセット数`}><button type="button" onClick={() => changePreviewSetCount(exercise.id, -1)} aria-label="セットを減らす">−</button><span>{exercise.sets} sets</span><button type="button" onClick={() => changePreviewSetCount(exercise.id, 1)} aria-label="セットを増やす">＋</button></div><button className="previewDragHandle" aria-label={`${exercise.name}を長押しして並べ替え`} onPointerDown={(event) => startPreviewDrag(event, exerciseIndex)} onPointerMove={movePreviewDrag} onPointerUp={endPreviewDrag} onPointerCancel={endPreviewDrag}>☰</button><button className="previewDelete" onClick={() => skipExercise(exercise.id)} aria-label={`${exercise.name}を削除`}>削除</button></div>
                  </div>
                );
              })}
              ...
              <section className="monthlyPlan historyCalendar card">
                ...
                <div className="monthCalendar">
                  ...
                  {weeklyPlan.map((day) => {
                    ...
                    const isStrategicRest = plannedPart === "休み" && day.date <= todayInJapan() && !actual;
                    return <button type="button" key={day.date} onClick={() => openCalendarDay(day)} className={`calendarDay${day.today ? " today" : ""}${actual ? " completed" : ""}${isStrategicRest ? " strategicRest" : ""}`}>
                      ...
                      {editingPlan ? (
                        ...
                        {plannedPart && !actual && <span className={`calendarPlanTag ${partTone(plannedPart)}`}>{plannedPart}</span>}
                        ...
                        {isStrategicRest && <span className="doneStamp restStamp" aria-label="戦略的休息">♥</span>}
                      </>}
                    </button>;
                  })}
                </div>
                  ...
                  return <article className="card historyCard" id={`history-${item.id}`} key={item.id}>
                    ...
                    {open && <div className="historyDetail">
                      {editingHistoryId !== item.id ? <>
                        ...
                        <div className="historyLocation historyLocationTopDisplay"><span>実施場所</span><strong>{meta.location || "PHGノース大阪"}</strong></div>
                        <div className="historyMetaBar"><div className="historyMetaStats"><span>開始 <b>{meta.start || "—"}</b></span><span>終了 <b>{meta.end || "—"}</b></span><span>評価 <b>{meta.assessment || "—"}</b></span><span>パンプ <b>{meta.pump || "—"}</b></span><span>Session Score <b className="historyStars">{meta.sessionScore ? `${"★".repeat(Number(meta.sessionScore))}${"☆".repeat(5 - Number(meta.sessionScore))}` : "—"}</b></span><span>ストレッチ <b>{meta.stretch || "—"}</b></span><span>腹筋 <b>{meta.abs || "—"}</b></span></div></div>
                        <div className="historyTableWrap"><table className="historyTable"><thead><tr><th>種目</th>{Array.from({length: Math.max(0, ...item.exercises.map((exercise) => exercise.sets.length))}, (_, index) => <th key={index}>{index + 1}セット目</th>)}</tr></thead><tbody>{item.exercises.map((exercise, exerciseIndex) => { const previousExercise = previousWorkout?.exercises?.find((row) => row.id === exercise.id); const setCountUp = previousExercise && exercise.sets.length > previousExercise.sets.length; return <tr className={setCountUp ? "historySetCountUp" : ""} key={exercise.id}><th><span className={`historyPartDot ${partTone(item.part)}`} />{exerciseIndex + 1}. {exercise.name}{setCountUp && <small>＋セット</small>}{cleanExerciseLogMemo(exercise.sets.find((set) => set.memo)?.memo) && <small className="historyExerciseMemoInline">メモ：{cleanExerciseLogMemo(exercise.sets.find((set) => set.memo)?.memo)}</small>}</th>{Array.from({length: Math.max(0, ...item.exercises.map((row) => row.sets.length))}, (_, index) => { const set = exercise.sets[index]; const previousSet = previousExercise?.sets?.[index]; const trend = compareHistorySet(set, previousSet); return <td className={trend} key={index}>{set ? <><strong>{set.weight}kg×{set.reps}回</strong>{previousSet && <small>前回 {previousSet.weight}kg×{previousSet.reps}</small>}{set.rir !== "" && set.rir != null ? <small>RIR {set.rir}</small> : null}</> : <span>—</span>}</td>; })}</tr>; })}</tbody></table></div>
                        {meta.cleanMemo && <p className="historyMemo">{meta.cleanMemo}</p>}<button className="secondary historyBottomEdit" onClick={() => beginHistoryEdit(item)}>✏️ ログを編集</button>
                        </>; })()}
                      </> : historyDraft && <div className="historyEditor">
                        <label className="historyLocationEdit historyLocationTop">実施場所<input type="text" value={historyDraft.location || ""} placeholder="PHGノース大阪" onChange={(event) => setHistoryDraft((draft) => ({ ...draft, location: event.target.value }))} /></label>
                        <div className="historyTimeGrid"><label>開始<input type="time" value={historyDraft.start || ""} onChange={(event) => setHistoryDraft((draft) => ({ ...draft, start: event.target.value }))} /></label><label>終了<input type="time" value={historyDraft.end || ""} onChange={(event) => setHistoryDraft((draft) => ({ ...draft, end: event.target.value }))} /></label></div>
                        <div className="historyRecoveryEdit"><label>ストレッチ<input value={historyDraft.stretch || ""} placeholder="例：5:20 / Skip" onChange={(event) => setHistoryDraft((draft) => ({ ...draft, stretch: event.target.value }))} /></label><label>腹筋<input value={historyDraft.abs || ""} placeholder="例：15回 / 15回" onChange={(event) => setHistoryDraft((draft) => ({ ...draft, abs: event.target.value }))} /></label><div><span>Session Score</span><div className="sessionScorePicker">{[1,2,3,4,5].map((score) => <button type="button" key={score} className={Number(historyDraft.sessionScore) >= score ? "active" : ""} onClick={() => setHistoryDraft((draft) => ({ ...draft, sessionScore: score }))}>★</button>)}</div></div></div>
                        ...
                        <button className="secondary danger historyWorkoutDelete" onClick={deleteHistoryWorkout} disabled={savingHistory}>このWorkoutを削除</button><div className="historyEditActions"><button className="secondary" onClick={cancelHistoryEdit}>キャンセル</button><button className="primary" onClick={saveHistoryEdit} disabled={savingHistory}>{savingHistory ? "保存中…" : "変更を保存"}</button></div>
                      </div>}
                    </div>}
                  </article>;
                })}</div>}
          ...
          {appView === "progress" && (
            <section className="analyticsPage">
              ...
              <nav className="progressTabs" aria-label="進捗カテゴリー">
                {[{id:"recovery",label:"回復"},{id:"exercise",label:"種目"},{id:"volume",label:"ボリューム"},{id:"body",label:"Body"}].map((tab) => <button key={tab.id} className={progressTab === tab.id ? "active" : ""} onClick={() => setProgressTab(tab.id)}>{tab.label}</button>)}
              </nav>
              ...
              {!analyticsBusy && analytics && <>
                {progressTab === "recovery" && <section className="recoverySection"><div className="recoveryIntro"><small>RECOVERY STATUS</small><h3>前回トレーニングからの経過</h3><p>部位ごとに、最後に実施してからの時間をまとめて確認できるで。</p></div><div className="partRestGrid progressPartRest">{partRest.map((item) => { const hours = item.latest ? Math.floor((now - workoutFinishedAt(item.latest)) / 3600000) : null; const status = hours == null ? "未実施" : hours < 48 ? "回復中" : hours < 96 ? "そろそろ" : hours < 168 ? "狙い目" : "かなり空き"; const recoveryRate = hours == null ? 0 : Math.min(100, hours / 168 * 100); const statusClass = hours == null ? "empty" : hours < 48 ? "recovering" : hours < 96 ? "soon" : hours < 168 ? "target" : "ready"; return <article className={`metric-card partRestCard recovery-${statusClass}`} key={item.part}><div className="recoveryCardHead"><strong>{item.part}</strong><span>{status}</span></div><b className="viz-stat-value">{item.elapsed}</b><div className="recoveryTrack"><i style={{width:`${recoveryRate}%`}} /></div><small>{item.latest ? `前回：${item.latest.date} ${extractWorkoutMeta(item.latest.memo).end || extractWorkoutMeta(item.latest.memo).start || ""}` : "まだ記録がないで"}</small></article>})}</div></section>}
          ...
          {appView === "coach" && <section className="coachPage analyticsPage">
            <div className="analyticsHead"><div><small>SMART COACH</small><h2>次回トレーニング提案</h2></div><span className={`coachPartBadge ${partTone(nextCoachPart)}`}>{nextCoachPart}</span></div>
            <nav className="coachPartTabs" aria-label="Coach部位切替">{PARTS.map((p) => <button key={p} className={nextCoachPart === p ? `active ${partTone(p)}` : partTone(p)} onClick={() => setCoachPart(p)}>{p}</button>)}<button className={!coachPart ? "active auto" : "auto"} onClick={() => setCoachPart("")}>次回</button></nav>
            <section className={`card smartCoach partAccent ${partTone(nextCoachPart)}`}><small>{coachPart ? "SELECTED PART" : "NEXT SESSION"}</small><h3>{!coachPart && nextCoachPlan ? `${nextCoachPlan.date.slice(5).replace("-", "/")}・${nextCoachPart}` : nextCoachPart}</h3><div className="coachStats"><span>前回 <b>{coachLatestWorkout?.date || "—"}</b></span><span>経過 <b>{coachDaysSince == null ? "—" : `${coachDaysSince}日`}</b></span><span>今月 <b>{coachMonthly ? `${coachMonthly.workouts}回 / ${coachMonthly.sets}set` : "—"}</b></span></div>{partCoachTips.length ? partCoachTips.map((tip) => <p key={tip}>{tip}</p>) : <p>{nextCoachPart}の記録がたまると、次回に絞った提案を表示するで。</p>}</section>
            <section className="card coachPriority"><small>PRIORITY</small><h3>次回の優先順位</h3>{partCoachTips.slice(0, 4).map((tip, index) => <div key={tip}><b>{index + 1}</b><span>{tip}</span></div>)}</section>
          </section>}
          {appView === "menu" && <section className="menuPage"><button>テンプレート編集 <span>›</span></button><button onClick={() => { openView("history"); setEditingPlan(true); }}>週間予定の編集 <span>›</span></button><button>Bodyサイズの記録 <span>›</span></button><button>写真の管理 <span>›</span></button><button>設定 <span>›</span></button><small>D-log Ver.10.8.0</small></section>}
          <nav className="bottomNav" aria-label="メインナビゲーション">
            ...
            <button className={appView === "coach" ? "active" : ""} onClick={() => openView("coach")}><b>✦</b><span>Coach</span></button>
      ...
      {preWorkoutOpen && (
        <div className="modalBackdrop"><section className="summaryModal card preWorkoutModal">
          <small>PRE-WORKOUT</small><h2>ストレッチタイマー</h2><p>実際に行った時間をそのまま記録するで。</p>
          <div className={`stretchTimer ${stretchStatus === "running" ? "running" : ""}`}><span>経過時間</span><strong>{formatTimer(stretchStartedAt ? Math.max(0, Math.floor((now - stretchStartedAt) / 1000)) : stretchSeconds)}</strong></div>
          <div className="stretchTimerActions">
            {stretchStatus === "running" ? <button className="secondary" type="button" onClick={pauseStretch}>一時停止</button> : <button className="primary" type="button" onClick={startStretch}>{stretchSeconds > 0 ? "再開" : "開始"}</button>}
            <button className="primary" type="button" onClick={finishStretchAndStart} disabled={stretchStatus === "pending" && stretchSeconds === 0}>終了してトレ開始</button>
          </div>
          <button className="secondary" onClick={skipStretchAndStart}>ストレッチなしで開始</button>
      ...
      {absPromptOpen && (
        <div className="modalBackdrop"><section className="summaryModal card absPromptModal">
          ...
          <div className="absSets fixedAbsSets">{absSets.map((set, index) => <label key={index} className={set.completed ? "completed" : ""}><input type="checkbox" checked={set.completed} onChange={(event) => setAbsSets((current) => current.map((item, i) => i === index ? {...item, reps:15, completed:event.target.checked} : item))}/><span>Set {index + 1}</span><strong>15回</strong></label>)}</div>
          ...
          <section className="summaryModal card saveConfirmModal">
            ...
            <div className="saveFields">
              ...
              <div className="simpleFeedback">
                ...
                <div><span>Session Score（5点満点）</span><div className="sessionScorePicker" role="radiogroup" aria-label="Session Score">{[1,2,3,4,5].map((score) => <button type="button" key={score} className={Number(saveConfirm.sessionScore) >= score ? "active" : ""} onClick={() => setSaveConfirm((current) => ({...current, sessionScore:score}))} aria-label={`${score}点`}>★</button>)}</div><p className="scoreDefinition">{({1:"最悪：ほとんど何もできなかった",2:"いまいち：予定より質が低かった",3:"普通：予定どおりできた",4:"良い：重量・回数・感覚が向上",5:"最高：自己ベスト級のセッション"})[saveConfirm.sessionScore] || "★を選ぶと定義を表示"}</p></div>
      ...
      {busy && saveConfirm && (
        <div className="savingOverlay" role="status" aria-live="assertive">
          <div className="savingCard">
            <div className="savingSpinner" aria-hidden="true" />
            <strong>Workoutを保存中…</strong>
            <span>記録とNotion同期が終わるまで、この画面のままで待ってな。</span>
          </div>
        </div>
      )}
        ...
        <div className="modalBackdrop">
          <section className="summaryModal card">
            ...
            <div className="summaryGrid">
              ...
              <div><span>Session Score</span><strong className="summaryStars">{summary.sessionScore ? `${"★".repeat(summary.sessionScore)}${"☆".repeat(5-summary.sessionScore)}` : "—"}</strong></div>
      ...
      {!workout && <footer className="appVersion">D-log Ver.10.8.0</footer>}
