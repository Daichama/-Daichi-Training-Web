...
function DLogo({ className = "dLogo" }) {
  ...
  function moveHistoryExercise(index, direction) {
    setHistoryDraft((draft) => {
      if (!draft) return draft;
      const nextIndex = index + direction;
      if (nextIndex < 0 || nextIndex >= draft.exercises.length) return draft;
    ...
    <main>
      <header className="brandHeader">
        <div className="brandLockup">
          <DLogo />
          <div>
            <div className="eyebrow">D-log · v9.2.1</div>
            <div className="brandTagline">MEASURE. IMPROVE. REPEAT.</div>
          </div>
        </div>
        <h1>{workout ? `${workout.part} Day` : appView === "history" ? "トレーニング履歴" : appView === "progress" ? "進捗" : "今日はどこやる？"}</h1>
      ...
      {!workout && (
        <>
          <nav className="appTabs" aria-label="D-log navigation">
            <button className={appView === "home" ? "active" : ""} onClick={() => openView("home")}>ホーム</button>
            <button className={appView === "history" ? "active" : ""} onClick={() => openView("history")}>履歴</button>
            <button className={appView === "progress" ? "active" : ""} onClick={() => openView("progress")}>進捗</button>
          </nav>
          {appView === "home" && <>
                ...
                {(analytics?.history || []).map((item) => {
                  ...
                  return <article className="card historyCard" id={`history-${item.id}`} key={item.id}>
                    <button className="historySummary" onClick={() => setSelectedHistoryId(open ? "" : item.id)}>
                      ...
                      <div className="historyStats"><span>{item.sets} sets</span><span>{formatVolume(item.volume)} kg</span><b>{open ? "−" : "+"}</b></div>
                    </button>
                    {open && <div className="historyDetail">
                      {editingHistoryId !== item.id ? <>
                        ...
                        <div className="historyTableWrap"><table className="historyTable"><thead><tr><th>種目</th>{Array.from({length: Math.max(0, ...item.exercises.map((exercise) => exercise.sets.length))}, (_, index) => <th key={index}>{index + 1}セット目</th>)}</tr></thead><tbody>{item.exercises.map((exercise) => <tr key={exercise.id}><th>{exercise.name}</th>{Array.from({length: Math.max(0, ...item.exercises.map((row) => row.sets.length))}, (_, index) => { const set = exercise.sets[index]; return <td key={index}>{set ? <><strong>{set.weight}kg×{set.reps}回</strong>{set.rir !== "" && set.rir != null ? <small>RIR {set.rir}</small> : null}</> : <span>—</span>}</td>; })}</tr>)}</tbody></table></div>
                        {item.memo && <p className="historyMemo">{item.memo}</p>}
                      </> : historyDraft && <div className="historyEditor">
                        <p className="historyEditHint">上下ボタンで実施順を変更。重量・回数・RIRも直接修正できるで。</p>
                        {historyDraft.exercises.map((exercise, exerciseIndex) => <section className="historyEditExercise" key={`${exercise.exerciseId}-${exerciseIndex}`}>
                          <div className="historyEditExerciseHead"><strong>{exerciseIndex + 1}. {exercise.name}</strong><div><button onClick={() => moveHistoryExercise(exerciseIndex, -1)} disabled={exerciseIndex === 0}>↑</button><button onClick={() => moveHistoryExercise(exerciseIndex, 1)} disabled={exerciseIndex === historyDraft.exercises.length - 1}>↓</button><button className="dangerText" onClick={() => removeHistoryExercise(exerciseIndex)}>種目削除</button></div></div>
                          <div className="historyEditSets">{exercise.sets.map((set, setIndex) => <div className="historyEditSet" key={set.id || setIndex}><span>Set {setIndex + 1}</span><label><input inputMode="decimal" value={set.weight ?? ""} onChange={(event) => updateHistorySet(exerciseIndex, setIndex, "weight", event.target.value)} />kg</label><label><input inputMode="numeric" value={set.reps ?? ""} onChange={(event) => updateHistorySet(exerciseIndex, setIndex, "reps", event.target.value)} />回</label><label>RIR <input inputMode="numeric" value={set.rir ?? ""} onChange={(event) => updateHistorySet(exerciseIndex, setIndex, "rir", event.target.value)} /></label><button className="historySetDelete" onClick={() => removeHistorySet(exerciseIndex, setIndex)}>×</button></div>)}</div>
                          <button className="secondary compact" onClick={() => addHistorySet(exerciseIndex)}>＋セット追加</button>
                        </section>)}
                        <div className="historyAddExercise"><select defaultValue="" onChange={(event) => { addHistoryExercise(event.target.value); event.target.value = ""; }}><option value="">＋種目を追加</option>{allExercises.filter((exercise) => !historyDraft.exercises.some((item) => item.exerciseId === exercise.id)).map((exercise) => <option key={exercise.id} value={exercise.id}>{exercise.name}</option>)}</select></div>
                        ...
                        <div className="historyEditActions"><button className="secondary" onClick={cancelHistoryEdit}>破棄</button><button className="primary" onClick={saveHistoryEdit} disabled={savingHistory}>{savingHistory ? "保存中…" : "変更を保存"}</button></div>
                      </div>}
                    </div>}
                  </article>;
                })}
