...
const DRAFT_KEY = "d-log-v9-0-0-draft";
    ...
    <main>
      <header className="brandHeader">
        <div className="brandLockup">
          <DLogo />
          <div>
            <div className="eyebrow">D-log · v9.1.0</div>
      ...
      {!workout && (
        <>
          ...
          <section className="monthlyPlan card">
            <div className="monthlyPlanHead">
              <button onClick={() => shiftCalendarMonth(-1)} aria-label="前の月">‹</button>
              <div><small>MONTHLY PLAN</small><strong>{calendarMonth.replace("-", "年")}月</strong></div>
            ...
            <div className="monthCalendar">
              ...
              {weeklyPlan.map((day) => {
                const completed = (analytics?.history || []).some((item) => item.date === day.date);
                return <div key={day.date} className={`calendarDay${day.today ? " today" : ""}${completed ? " completed" : ""}`}>
                  <div className="calendarDate"><span>{Number(day.date.slice(8))}</span>{completed && <b>✓</b>}</div>
                  {editingPlan ? (
                    <select value={day.menu || ""} onChange={(event) => changePlanDay(day.date, event.target.value)}>
            ...
            <section className="analyticsPage">
              ...
              <div className="historyList">
                {(analytics?.history || []).map((item) => {
                  ...
                  return <article className="card historyCard" key={item.id}>
                    ...
                    {open && <div className="historyDetail">
                      <div className="historyTableWrap"><table className="historyTable"><thead><tr><th>種目</th>{Array.from({length: Math.max(0, ...item.exercises.map((exercise) => exercise.sets.length))}, (_, index) => <th key={index}>{index + 1}セット目</th>)}</tr></thead><tbody>{item.exercises.map((exercise) => <tr key={exercise.id}><th>{exercise.name}</th>{Array.from({length: Math.max(0, ...item.exercises.map((row) => row.sets.length))}, (_, index) => { const set = exercise.sets[index]; return <td key={index}>{set ? <><strong>{set.weight}kg×{set.reps}回</strong>{set.rir ? <small>RIR {set.rir}</small> : null}</> : <span>—</span>}</td>; })}</tr>)}</tbody></table></div>
