...
function exerciseOrderFromMemo(text) {
  const match = String(text || "").match(/\[DLOG_EXERCISE_ORDER=(\d+)\]/);
...
export async function GET(request) {
  try {
    ...
    // 胸・腕などの開始画面は「直近に実際に行った種目順」をデフォルトにする。
    // これでNotionマスターの古い順番に戻らず、最新セッションの並びを維持する。
    const masterPartById = new Map(masters.map((page) => [page.id, selectName(page.properties?.["部位"])]));
    let latestPerformedDate = "";
    const latestPerformedOrder = new Map();
    if (!all) {
      for (const page of logs) {
        const props = page.properties || {};
        const exerciseId = relationIds(props["種目リンク"] || props["種目"])[0];
        const exercisePart = masterPartById.get(exerciseId) || selectName(props["部位"]);
        if (!(exercisePart === part || (part === "腕" && exercisePart === "ウォームアップ"))) continue;
        const date = String(props["日付"]?.date?.start || "").slice(0,10);
        const order = exerciseOrderFromMemo(plain(props["メモ"]));
        if (!date || !order) continue;
        if (date > latestPerformedDate) { latestPerformedDate = date; latestPerformedOrder.clear(); }
        if (date === latestPerformedDate && !latestPerformedOrder.has(exerciseId)) latestPerformedOrder.set(exerciseId, order);
      }
    }
    ...
    const exercises = masters
      ...
      .sort((a, b) => {
        if (!all && latestPerformedOrder.size) {
          const ao = latestPerformedOrder.get(a.id);
          const bo = latestPerformedOrder.get(b.id);
