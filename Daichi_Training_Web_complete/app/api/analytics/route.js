  ...
  try {
    ...
    const history = Array.from(groupedWorkouts.values()).map((workout) => {
      ...
      for (const log of workout.logs) {
        if (!exercises.has(log.exerciseId)) exercises.set(log.exerciseId, { id: log.exerciseId, name: log.exerciseName, sets: [] });
        exercises.get(log.exerciseId).sets.push(log);
      }
      return {
        id: workout.id,
        date: workout.date,
        ...
        exercises: Array.from(exercises.values()).map((item) => ({ ...item, sets: item.sets.sort((a, b) => a.setNo - b.setNo) })),
      };
    }).sort(sortDateDesc);
    ...
    return NextResponse.json({ ok: true, overview, history, exercises });
  } catch (error) {
