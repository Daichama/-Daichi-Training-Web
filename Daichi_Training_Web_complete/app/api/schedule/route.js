...
import { DB, plain, queryAll, selectName, createPage, updatePage, archivePage, titleValue, richValue, selectValue, dateValue } from "../../lib/notion";
...
const FALLBACK = {
  ...
  "2026-08-08": { menu: "休み", onCall: false },
...
export async function GET(request) {
  ...
  if (DB.schedule) {
    try {
      ...
      const sortedPages = [...pages].sort((a,b) => String(b.last_edited_time || b.created_time || "").localeCompare(String(a.last_edited_time || a.created_time || "")));
      const seen = new Set();
      rows = sortedPages.map((page) => {
        const props = page.properties || {};
        return { date: firstDate(props), menu: firstText(props, ["メニュー", "部位", "Workout", "予定", "名前", "Name"]), onCall: boolValue(props, ["当直", "On-call", "勤務", "メモ"]) };
      }).filter((row) => row.date && !seen.has(row.date) && seen.add(row.date));
    } catch { rows = []; }
  }
...
export async function POST(request) {
  try {
    ...
    for (const page of pages) {
      const date = firstDate(page.properties || {});
      if (!date) continue;
    ...
    for (const [date, entries] of grouped) {
      entries.sort((a,b) => String(b.last_edited_time || b.created_time || "").localeCompare(String(a.last_edited_time || a.created_time || "")));
      existing.set(date, entries[0]);
    }
    let updated = 0;
    let created = 0;
    for (const item of days) {
      ...
      const duplicates = (grouped.get(date) || []).filter((entry) => entry.id !== page?.id);
      for (const duplicate of duplicates) { try { await archivePage(duplicate.id); } catch {} }
      const properties = {
        "日付": dateValue(date),
