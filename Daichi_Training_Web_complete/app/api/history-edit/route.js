...
export async function DELETE(request) {
  try {
    ...
    if (!workoutId) return NextResponse.json({ error: "Workout IDがありません" }, { status: 400 });
    for (const id of logIds) {
      if (id && !String(id).startsWith("tmp-")) await archivePage(id);
    }
    await archivePage(workoutId);
    return NextResponse.json({ ok: true, deletedLogs: logIds.length });
  } catch (error) {
    return NextResponse.json({ error: error instanceof Error ? error.message : "Workoutを削除できませんでした" }, { status: 500 });
  }
}
