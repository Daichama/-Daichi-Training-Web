...
export async function POST(request) {
  try {
    ...
    const sessionMarker = safeSessionId ? `[DLOG_SESSION_ID=${safeSessionId}]` : "";
    const workoutMemo = [timeMemo, ratingMemo, String(sessionMemo || "").trim(), sessionMarker].filter(Boolean).join(" / ");

    if (safeSessionId) {
      if (activeSessions.has(safeSessionId)) {
        return NextResponse.json({ error: "このWorkoutは保存処理中です" }, { status: 409 });
      }
      activeSessions.add(safeSessionId);
      setTimeout(() => activeSessions.delete(safeSessionId), 120000);
      const existing = await queryAll(DB.workout, {
        filter: { property: "メモ", rich_text: { contains: sessionMarker } },
        page_size: 1,
      });
      if (existing.length) {
        activeSessions.delete(safeSessionId);
        return NextResponse.json({ ok: true, duplicatePrevented: true, workout: { id: existing[0].id, url: existing[0].url, date, part }, savedSets: 0, failedSets: 0, updated: 0, created: 0, syncError: "" });
      }
    }
    ...
    if (safeSessionId) activeSessions.delete(safeSessionId);
    return NextResponse.json({
      ok: true,
      workout: {
        id: workoutPage.id,
