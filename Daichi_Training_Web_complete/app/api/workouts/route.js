...
export async function POST(request) {
  try {
    ...
    const ratingMemo = [
      ratings?.fatigue ? `しんどさ ${ratings.fatigue}/5` : "",
      ratings?.satisfaction ? `充実度 ${ratings.satisfaction}/5` : "",
      ratings?.pump ? `パンプ ${ratings.pump}/5` : "",
    ].filter(Boolean).join(" / ");
    const workoutMemo = ["D-log v6.1から一括保存", timeMemo, ratingMemo, String(sessionMemo || "").trim()].filter(Boolean).join(" / ");
    ...
    // 種目の実施順は、各種目で最初に完了したセットの時刻を優先する。
    // 古いドラフトなど時刻がない場合だけ、画面上の並び順を使う。
